...
export async function getOverview(req, res) {
  const [
    ...
    query(`SELECT ff.user_id,
              COALESCE(u.username, u.full_name, u.email) AS name,
              u.email,
              COUNT(*)::int AS flag_count,
              MAX(ff.severity)::int AS max_severity
           FROM fraud_flags ff
           JOIN users u ON u.id = ff.user_id
           WHERE ff.status IN ('open','reviewing')
           GROUP BY ff.user_id, u.username, u.full_name, u.email
           ORDER BY max_severity DESC, flag_count DESC
           LIMIT 10`),
